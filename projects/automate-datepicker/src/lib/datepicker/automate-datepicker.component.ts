...
import { AutomateDatePickerCalendarComponent } from './automate-datepicker-calendar/automate-datepicker-calendar.component';
...
export class AutomateDatePickerComponent implements OnDestroy, ControlValueAccessor {
  ...
  public daySelected(day: Day): void {
    ...
    date.setMonth(day.dayDate.getMonth(), day.dayDate.getDate());
  ...
  private _appendComponentToBody(componentRef: ComponentRef<AutomateDatePickerDropdownComponent>) {
    ...
    // 4. Run change detection: first tick lets dropdown ngOnInit run (sets positionStyle), second tick renders template with footer
    this._appRef.tick();
  }

  private _updateFormattedDate(): void {
    this.formattedDate = this._model ? DateHelper.toFormat(this._model, this._configOrDefault.format ?? Constants.DEFAULT_DATE_FORMAT) : null;
  }
