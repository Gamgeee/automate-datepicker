...
export class Year {
    ...
    constructor(yearNumber: number, config: DatePickerConfig, createMonths: boolean = true, createWeeks: boolean = true) {
        ...
        this._yearLabel = DateHelper.toFormat(new Date(this._yearNumber, 1, 1), config.yearLabel ?? Constants.DEFAULT_YEAR_FORMAT);
        this._yearTitle = DateHelper.toFormat(new Date(this._yearNumber, 1, 1), config.yearTitle ?? Constants.DEFAULT_YEAR_FORMAT);
