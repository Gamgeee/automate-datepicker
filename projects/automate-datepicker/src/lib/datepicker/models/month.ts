...
export class Month {
    ...
    constructor(yearNumber: number, monthNumber: number, config: DatePickerConfig, createWeeks: boolean = true) {
        ...
        this._monthLabel = DateHelper.toFormat(new Date(this._yearNumber, this._monthNumber, 1), config.monthLabel ?? Constants.DEFAULT_MONTH_FORMAT);
        this._monthTitle = DateHelper.toFormat(new Date(this._yearNumber, this._monthNumber, 1), config.monthTitle ?? Constants.DEFAULT_MONTH_FORMAT);
    ...
    public updateDisabledState(config: DatePickerConfig): void {
        ...
        this._isDisabled = Boolean(isDisabledByMinDate || isDisabledByMaxDate);
